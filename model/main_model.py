...
class HYPER_DANIEL(nn.Module):
    def __init__(self, config):
        """
            The implementation of DANIEL with hypernetwork-based actor for multi-objective learning
        :param config: a package of parameters
        """
        ...
        # Use the same feature extraction network as DANIEL
        self.feature_exact = DualAttentionNetwork(config).to(device)

        # Determine preference dimension from number of objectives
        pref_dim = (
            len(config.objective_fn) if isinstance(config.objective_fn, list) else 2
        )

        # Use HyperActor instead of regular Actor
        self.actor = HyperActor(
            ...
            output_dim=1,
            pref_dim=pref_dim,
            hyper_hidden_dim=256,  # Following example pattern
            embd_dim=2,  # Following example pattern
        ).to(device)
    ...
    def assign_preferences(self, preferences):
        """
        Assign preferences to the hypernetwork to generate actor parameters.
        This should be called once per episode/rollout with all preferences in the batch.

        :param preferences: preference vectors [batch_size, pref_dim]
                           Each sample in the batch can have a different preference
        """
    ...
    def forward(
        ...
        :param dynamic_pair_mask: a tensor with shape [sz_b, J, M], used for masking
                            incompatible op-mch pairs
        :param fea_pairs: pair features with shape [sz_b, J, M, 8]
        :param preferences: IGNORED during rollout - actor parameters should be pre-generated via assign_preferences()
                           Used during training to regenerate parameters for each minibatch
        :param preference_indices: indices mapping each sample to its preference [batch_size]
                                   Used for memory-efficient validation with unique preferences
        :param fea_j_params: learnable job feature parameters for active search [num_preferences, hidden_dim]
                            Used to enable gradient-based optimization of preference-specific features
        :param fea_m_params: learnable machine feature parameters for active search [num_preferences, hidden_dim]
                            Used to enable gradient-based optimization of preference-specific features
        :return:
            pi: scheduling policy with shape [sz_b, J*M]
            v: the value of state with shape [sz_b, 1] or [sz_b, pref_dim]
        """
        # Feature extraction (same as DANIEL)
        fea_j, fea_m, fea_j_global, fea_m_global = self.feature_exact(
            fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx
        )
        ...
        if fea_j_params is not None:
            ...
            # Select appropriate parameter matrix for each sample
            # fea_j_params: [num_prefs, N_ops, hidden_dim]
            # preference_indices: [sz_b]
            # Result: [sz_b, N_ops, hidden_dim]
            selected_fea_j_params = fea_j_params[
                preference_indices
            ]  # [sz_b, N_ops, hidden_dim]

            # Add to per-operation features (direct addition, shapes match exactly)
            fea_j = (
                fea_j + selected_fea_j_params
            )  # [sz_b, N_ops, hidden_dim] + [sz_b, N_ops, hidden_dim]

            # Add to global job features (average the per-operation parameters)
            fea_j_global = fea_j_global + selected_fea_j_params.mean(
                dim=1
            )  # [sz_b, hidden_dim]
        ...
        if fea_m_params is not None:
            ...
            # Result: [sz_b, N_machines, hidden_dim]
            selected_fea_m_params = fea_m_params[
                preference_indices
            ]  # [sz_b, N_machines, hidden_dim]

            # Add to per-machine features (direct addition, shapes match exactly)
            fea_m = (
                fea_m + selected_fea_m_params
            )  # [sz_b, N_machines, hidden_dim] + [sz_b, N_machines, hidden_dim]

            # Add to global machine features (average the per-machine parameters)
            fea_m_global = fea_m_global + selected_fea_m_params.mean(
                dim=1
            )  # [sz_b, hidden_dim]
        ...
        # HyperActor: uses pre-generated parameters from assign_preferences() during rollout
        # During PPO update, preferences are passed and parameters regenerated for each minibatch
        # During memory-efficient validation, preference_indices map samples to unique preferences
