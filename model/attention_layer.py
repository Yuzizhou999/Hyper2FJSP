...
class SingleOpAttnBlock(nn.Module):
    ...
    def forward(self, h, op_mask):
        ...
        Wh2_concat = torch.stack(
            [Wh2.roll(1, dims=1), Wh2, Wh2.roll(-1, dims=1)], dim=-1
        )
...
class MultiHeadOpAttnBlock(nn.Module):
    def __init__(
        self, input_dim, output_dim, dropout_prob, num_heads, activation, concat=True
    ):
        """
        ...
        self.attentions = [
            SingleOpAttnBlock(input_dim, output_dim, dropout_prob)
...
class MultiHeadMchAttnBlock(nn.Module):
    def __init__(
        ...
        self.attentions = [
            SingleMchAttnBlock(node_input_dim, edge_input_dim, output_dim, dropout_prob)
            for _ in range(num_heads)
        ]
        for i, attention in enumerate(self.attentions):
            self.add_module("attention_{}".format(i), attention)
