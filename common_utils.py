...
def isin_2d(a: np.array, b: np.array):
    """2-dimensional variant of numpy isin function. a is a 1D array and b is a 2D array.
    For each element in a, check if it is in the corresponding row of b.

    Args:
        a (np.array): The 1D input array to check elements from.
        b (np.array): The 2D reference array to check against.
    """
...
def find_pareto_efficient_solutions(costs):
    """
    Find the pareto-efficient points
    :param costs: An (n_points, n_costs) array
    :return: An array of indices of pareto-efficient points.
    """
    selection_mask = is_pareto_efficient(costs)
    pareto_efficient_solutions = costs[selection_mask]
    return pareto_efficient_solutions
