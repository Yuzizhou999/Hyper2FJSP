...
class MOFJSPEnvForSameOpNums(FJSPEnvForSameOpNums):
    def __init__(
        ...
        :param use_simple_reward: whether to use simple actual-value reward (training-only option)
        :param use_lb_features: whether to use lower bound features (op_ct_lb, op_lateness_lb, op_flowtime_lb)
        """
        ...
        self.mch_fea_dim = 8
        # The objectives functions to be optimized
    ...
    def compute_reward(self) -> np.ndarray:
        """
        Compute reward based on the environment configuration.

        :return: reward array - either multi-objective [num_envs, num_objectives]
        """
        if self.use_simple_reward:
    ...
    def _compute_multi_objective_reward(self) -> np.ndarray:
        """
        Compute multi-objective rewards (original implementation).
