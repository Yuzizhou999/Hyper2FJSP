...
def draw_gantt_chart(JobShop):
    ...
    ax.set_yticklabels(
        [f"M{machine_id}" for machine_id in range(JobShop.nr_of_machines)]
    )
    ax.set_xlabel("Time")
...
def draw_precedence_relations(JobShop: JobShop):
    ...
    for key, value in middle_nodes.items():
        if (key % 2) == 0:
            ...
            pos.update(
                {n: (max_length - key + 1, i + 0.5) for i, n in enumerate(value)}
            )
    ...
    for node in G.nodes:
        options["node_color"].append(
            colormap(JobShop.get_operation(node).job_id % colormap.N)
        )
