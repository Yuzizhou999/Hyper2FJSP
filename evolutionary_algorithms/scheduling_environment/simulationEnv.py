        ...
        if machine.machine_id in operation.processing_times:
            with self.machine_resources[machine.machine_id].request() as req:
                ...
                machine.add_operation_to_schedule_at_time(
                    operation, start_time, processing_time, setup_time=0
                )
    ...
    def generate_online_job_arrivals(self):
        ...
        while True:
            ...
            num_operations = random.randint(
                self.min_nr_operations_per_job, self.max_nr_operations_per_job
            )
            for i in range(num_operations):
                ...
                for machine_id in range(self.JobShop.nr_of_machines):
                    duration = random.randint(
                        self.min_duration_per_operation, self.max_duration_per_operation
                    )
