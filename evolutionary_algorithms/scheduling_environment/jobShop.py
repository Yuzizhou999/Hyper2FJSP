...
class JobShop:
    ...
    def reset(self):
        ...
        self._operations_to_be_scheduled = [operation for operation in self._operations]
    ...
    def add_precedence_relations_jobs(
        self, precedence_relations_jobs: Dict[int, List[int]]
    ) -> None:
    ...
    def add_precedence_relations_operations(
        self, precedence_relations_operations: Dict[int, List[int]]
    ) -> None:
        """Add precedence relations between operations."""
        self._precedence_relations_operations = precedence_relations_operations

    def add_sequence_dependent_setup_times(
        self, sequence_dependent_setup_times: List
    ) -> None:
        """Add sequence dependent setup times."""
        self._sequence_dependent_setup_times = sequence_dependent_setup_times

    def set_operations_available_for_scheduling(
        self, operations_available_for_scheduling: List
    ) -> None:
    ...
    def set_max_duration(self, max_duration: int) -> None:
        """Set the max duration of the operations."""
        self._max_duration = max_duration

    @property
    def max_duration(self) -> int:
        """Return the max duration of the operations."""
    ...
    def max_workload(self) -> float:
        ...
        return max(
            sum(op.scheduled_duration for op in machine.scheduled_operations)
            for machine in self.machines
        )
    ...
    def total_tardiness(self, deadline_alpha=0.5) -> float:
        total_tardiness = 0
        for job in self._jobs:
            job_min_processing_times = 0
            for operation in job.operations:
                job_min_processing_times += min(operation.processing_times.values())
            job_completion_time = max(
                operation.scheduled_end_time for operation in job.operations
            )
            tardiness = max(
                0, job_completion_time - job_min_processing_times * (1 + deadline_alpha)
            )
            total_tardiness += tardiness
        return total_tardiness

    def total_earliness(self, deadline_alpha=0.5) -> float:
        total_earliness = 0
        for job in self._jobs:
            job_min_processing_times = 0
            for operation in job.operations:
            ...
            deadline = job_min_processing_times * (1 + deadline_alpha)
            earliness = max(0, deadline - job_completion_time)
            total_earliness += earliness
        return total_earliness

    def total_costs(self) -> float:
        return sum(
            self.max_duration - op.scheduled_duration
            for op in self.scheduled_operations
        )

    def schedule_operation_with_backfilling(
        self, operation: Operation, machine_id, duration
    ) -> None:
    ...
    def schedule_operation_on_machine_backfilling(
        self, operation: Operation, machine_id, duration
    ) -> None:
    ...
    def mark_operation_as_available(self, operation: Operation) -> None:
        ...
        if operation not in self.scheduled_operations:
            raise ValueError(f"Operation {operation.operation_id} is not scheduled")
