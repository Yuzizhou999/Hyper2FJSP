...
class Machine:
    def __init__(self, machine_id, machine_name=None):
        ...
        self._old_processed_operations = None
    ...
    def scheduled_operations(self) -> List[Operation]:
        ...
        if self._old_processed_operations == self._processed_operations:
            ...
            self._old_processed_operations = self._processed_operations.copy()
            self._scheduled_operations = sorted_operations
            return self._scheduled_operations

    def add_operation_to_schedule_at_time(
        self, operation, start_time, processing_time, setup_time
    ):
    ...
    def add_operation_to_schedule_backfilling(
        self, operation: Operation, processing_time, sequence_dependent_setup_times
    ):
        ...
        finishing_time_machine = max(
            [operation.scheduled_end_time for operation in self.scheduled_operations],
            default=0,
        )

        setup_time = 0
        if len(self.scheduled_operations) != 0:
            setup_time = sequence_dependent_setup_times[self.machine_id][
                self.scheduled_operations[-1].operation_id
            ][operation.operation_id]

        # # find backfilling opportunity
        start_time_backfilling, setup_time_backfilling = (
            self.find_backfilling_opportunity(
                operation,
                finishing_time_predecessors,
                processing_time,
        ...
        if start_time_backfilling is not None:
            ...
            start_time = max(
                finishing_time_predecessors, finishing_time_machine + setup_time
            )
    ...
    def find_backfilling_opportunity(
        self,
        operation,
        finishing_time_predecessors,
        duration,
        ...
        if len(self.scheduled_operations) > 0:
            ...
            if (
                ...
                next_operation.update_sequence_dependent_setup_times(
                    next_operation.scheduled_start_time
                    - sequence_dependent_setup_times[self.machine_id][
                        operation.operation_id
                    ][self.scheduled_operations[0].operation_id],
                    sequence_dependent_setup_times[self.machine_id][
                ...
                for i in range(1, len(self.scheduled_operations[1:])):
                    ...
                    if (
                        ...
                        - self.scheduled_operations[i - 1].scheduled_end_time
                    ) >= duration + sequence_dependent_setup_times[self.machine_id][
                        self.scheduled_operations[i - 1].operation_id
                    ][operation.operation_id] + sequence_dependent_setup_times[
                        self.machine_id
                    ][operation.operation_id][
                        ...
                        if (
                            ...
                            self.scheduled_operations[
                                i
                            ].update_sequence_dependent_setup_times(
                                self.scheduled_operations[i].scheduled_start_time
                                - sequence_dependent_setup_times[self.machine_id][
                                    operation.operation_id
                                ][self.scheduled_operations[i].operation_id],
                            ...
                            return self.scheduled_operations[
                                i - 1
                            ].scheduled_end_time + sequence_dependent_setup_times[
                                self.machine_id
                            ][self.scheduled_operations[i - 1].operation_id][
                                operation.operation_id
                            ], sequence_dependent_setup_times[self.machine_id][
